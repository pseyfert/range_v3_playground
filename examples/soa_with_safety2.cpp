...
int main() {
  ...
  Zipping::ZipContainer<SOA::Container<std::vector, s_fitres>>  foo4{};
  ...
  try {
    [[maybe_unused]] auto x =
        Zipping::semantic_zip<s_track_with_fitres>( foo1, foo4 ); // works at compile time, assertion failure at run time
  } catch ( const std::exception& e ) { std::cout << "caught expected exception: " << e.what() << std::endl; }
  try {
    [[maybe_unused]] auto x =
        Zipping::semantic_zip<s_track_with_fitqual>( foo1, foo3 ); // works at compile time, throws at run time
  } catch ( const std::exception& e ) { std::cout << "caught expected exception: " << e.what() << std::endl; }
  {
