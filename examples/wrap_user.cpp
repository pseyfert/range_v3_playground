...
typename std::enable_if<
    SOA::Utils::ALL(
        std::is_same<typename std::remove_cv<typename std::remove_reference<typename CORE::value_type>::type>::type,
                     typename std::remove_cv<
                         typename std::remove_reference<typename EXTS::value_type::extends_t>::type>::type>::value... ),
...
template <
    template <typename> typename SKIN, typename CORE, typename... EXTS,
    typename = typename std::enable_if<SOA::Utils::ALL(
        SOA::impl::is_skin<SKIN>(),
        std::is_same<typename std::remove_cv<typename std::remove_reference<typename CORE::value_type>::type>::type,
                     typename std::remove_cv<typename std::remove_reference<
                         typename EXTS::value_type::extends_t>::type>::type>::value... )>>
auto zipsoa( CORE c, EXTS... e ) {
#ifndef NDEBUG
  if ( !variable_length_and( e.validate_ref( &c )... ) ) throw IncompatibleZip( "extension extends other core" );
#endif
...
//             VIEWS::contained_type::type::extends_t>::type>::type>::value... ),
...
int main() {
  ...
  for ( size_t i = 0; i < v.size(); ++i ) { std::cout << "\t" << v[i] << "\t" << vc[i] << std::endl; }
  ...
  try {
    ...
    return zv.size();
  } catch ( IncompatibleZip i ) { std::cout << i.message() << std::endl; }
}
