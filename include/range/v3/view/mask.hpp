        ...
        namespace view
        {
            ...
            struct make_vector_or_mask_fn
            {
                   ...
                   Msks_&& msks) const // TODO const and reference and stuff
               {
                   ...
                   // // todo return range with all true (an or of zero elements
                   // // is true)
                   assert(msks.size() != 0);
                   // if(msks.size() == 1)
                   //   return msks[0];
                   ...
                   // if(msks.size() == 5)
                   //   return or_ranges(msks[0], msks[1], msks[2], msks[3], msks[4]);
                   // if(msks.size() == 6)
                   //   return or_ranges(msks[0], msks[1], msks[2], msks[3], msks[4], msks[5]);
                   // if(msks.size() == 7)
                   //   return or_ranges(msks[0], msks[1], msks[2], msks[3], msks[4], msks[5], msks[6]);
                   // if(msks.size() == 8)
                   //   return or_ranges(msks[0], msks[1], msks[2], msks[3], msks[4], msks[5], msks[6], msks[7]);
                   ranges::v3::any_view<bool> partial = this->operator()(ranges::view::slice(msks,7,ranges::end));
                   return or_ranges(msks[0], msks[1], msks[2], msks[3], msks[4], msks[5], msks[6], std::move(partial));
//                //     // return ranges::view::zip(std::forward<Msks>(msks)...)
//                //     |
//                //     //        ranges::view::transform(
//                //     //            [](auto&& range_item) -> bool {
