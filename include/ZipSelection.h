...
public:
    ...
    NotGaudiException(std::string s) : m_message(std::move(s)) {}
...
namespace details {
                                 ...
                                 const char* msg_prefix)
    {
        if (UNLIKELY(s1.m_container.data() != s2.m_container.data())) {
            throw NotGaudiException{std::string{msg_prefix} +
...
struct SelectionView {
            ...
            : m_container{container}
    {
        ...
        } else if constexpr (std::is_same_v<Predicate,
                                            details::alwaysFalse>) {
    ...
    {
        return SelectionView{*this, std::forward<Predicate>(p),
                             reserveCapacity};
    }
                                   ...
                                   SelectionView const& s2)
    {
        ...
        SelectionView ret{
                s1.m_container, details::alwaysFalse{},
                static_cast<int>(std::min(est_csize, s1.size() + s2.size()))};

        // Use std::set_union to combine s1.m_indices and s2.m_indices into a
        // new index container
        std::set_union(s1.m_indices.begin(), s1.m_indices.end(),
                       s2.m_indices.begin(), s2.m_indices.end(),
                                          ...
                                          SelectionView const& s2)
    {
        ...
        std::set_intersection(s1.m_indices.begin(), s1.m_indices.end(),
                              s2.m_indices.begin(), s2.m_indices.end(),
                              std::back_inserter(ret.m_indices));
        return ret;
    }
                                        ...
                                        SelectionView const& s2)
    {
        ...
        // into a new index container
        std::set_difference(s1.m_indices.begin(), s1.m_indices.end(),
                            s2.m_indices.begin(), s2.m_indices.end(),
                            std::back_inserter(ret.m_indices));
        return ret;
    }
    ...
    friend bool includes(SelectionView const& s1, SelectionView const& s2)
    {
        ...
        return std::includes(s1.m_indices.begin(), s1.m_indices.end(),
                             s2.m_indices.begin(), s2.m_indices.end());
    }
                                                  ...
                                                  SelectionView const& s2)
    {
        ...
        std::set_symmetric_difference(
                s1.m_indices.begin(), s1.m_indices.end(),
                s2.m_indices.begin(), s2.m_indices.end(),
                std::back_inserter(ret.m_indices));
        return ret;
    }
...
namespace details {
    ...
    template <typename S>
    inline constexpr bool isSelection_v =
            details::isSelectionHelper<S>::value;
} // namespace details
