...
namespace details
{
    ...
    template<typename T>
    inline auto const typename_v =
        boost::typeindex::type_id_with_cvr<T>().pretty_name();

    template<typename S>
    void check_for_set_operation(S const& s1, S const& s2,
                                 const char* msg_prefix)
    {
        if(UNLIKELY(s1.m_container.data() != s2.m_container.data()))
        {
            throw NotGaudiException{
                ...
                "(SelectionView, SelectionView) was given views to different "
                "underlying storage." +
                typename_v<S>};
        }
...
struct SelectionView
{
    ...
    struct const_iterator
    {
        ...
        friend difference_type operator-(const_iterator const& lhs,
                                         const_iterator const& rhs)
        {
     ...
     *
     *  @param container       Contiguous storage container that this selection
     * refers to.
     *  @param predicate       Optional predicate applied to elements of the
     * input container.
     *  @param reserveCapacity Optional estimate of the number of elements that
     * will be selected by predicate, which is used to initialise the vector of
     * selected indices.
     */
    ...
    {
        if(UNLIKELY(m_container.size() - 1 >
                    std::numeric_limits<IndexSize>::max()))
        {
            throw NotGaudiException{
                "Index overflow: " + std::to_string(container.size() - 1) +
        ...
        else if constexpr(std::is_same_v<Predicate, details::alwaysFalse>)
        {
            ...
            {
                m_indices.reserve(reserveCapacity);
            }
        }
        else
        {
            m_indices.reserve(reserveCapacity < 0 ? m_container.size()
                                                  : reserveCapacity);
            auto offset = 0;
      ...
      : m_container{old_selection.m_container}
    {
        ...
        // don't need to do any overflow checking apply an additional selection
        // on the input selection
        m_indices.reserve(reserveCapacity < 0 ? old_selection.size()
                                              : reserveCapacity);
        std::copy_if(
            old_selection.m_indices.begin(),
    ...
    {
        return {m_container, m_indices.begin()};
    }

    const_iterator end() const
    {
        return {m_container, m_indices.end()};
    }
    ...
    SelectionView select(Predicate&& p, int reserveCapacity = -1) const
    {
        return SelectionView{
            *this, std::forward<Predicate>(p), reserveCapacity};
    }
    ...
    // same containerset
    friend SelectionView set_union(SelectionView const& s1,
                                   SelectionView const& s2)
    {
        details::check_for_set_operation(
        ...
        std::size_t est_csize =
            std::max(s1.m_indices.back(), s2.m_indices.back()) + 1;
        SelectionView ret{
            s1.m_container,
            details::alwaysFalse{},
        ...
        // new index container
        std::set_union(s1.m_indices.begin(),
                       s1.m_indices.end(),
                       s2.m_indices.begin(),
                       s2.m_indices.end(),
    ...
    friend SelectionView set_intersection(SelectionView const& s1,
                                          SelectionView const& s2)
    {
        details::check_for_set_operation(
            s1,
            s2,
            "set_intersection"); // check s1 and s2 are valid and compatible

        // Create a new SelectionView object with an empty index vector with an
        // appropriate reserved capacity
        SelectionView ret{s1.m_container,
                          details::alwaysFalse{},
                          static_cast<int>(std::min(s1.size(), s2.size()))};

        // Use std::set_intersection to combine s1.m_indices and s2.m_indices
        // into a new index container
        std::set_intersection(s1.m_indices.begin(),
                              s1.m_indices.end(),
                              s2.m_indices.begin(),
                              s2.m_indices.end(),
                              std::back_inserter(ret.m_indices));
    ...
    friend SelectionView set_difference(SelectionView const& s1,
                                        SelectionView const& s2)
    {
        details::check_for_set_operation(
            s1,
        ...
        // Use std::set_difference to combine s1.m_indices and s2.m_indices into
        // a new index container
        std::set_difference(s1.m_indices.begin(),
                            s1.m_indices.end(),
                            s2.m_indices.begin(),
                            s2.m_indices.end(),
                            std::back_inserter(ret.m_indices));
    ...
    {
        details::check_for_set_operation(
            s1, s2, "includes"); // check s1 and s2 are valid and compatible
        return std::includes(s1.m_indices.begin(),
                             s1.m_indices.end(),
                             s2.m_indices.begin(),
                             s2.m_indices.end());
    }

    friend SelectionView set_symmetric_difference(SelectionView const& s1,
                                                  SelectionView const& s2)
    {
        details::check_for_set_operation(
            s1,
        ...
        // Use std::set_difference to combine s1.m_indices and s2.m_indices into
        // a new index container
        std::set_symmetric_difference(s1.m_indices.begin(),
                                      s1.m_indices.end(),
                                      s2.m_indices.begin(),
                                      s2.m_indices.end(),
                                      std::back_inserter(ret.m_indices));
...
// Value-by-value comparison, don't care whether lhs and rhs are actually views
// onto the same container.
template<typename S, typename = requireSelection<S>>
bool
equal_values(S const& lhs, S const& rhs)
{
...
template<typename S, typename = requireSelection<S>>
bool
equal_values(S const& lhs, typename S::container_t rhs)
{
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}
...
template<typename S, typename = requireSelection<S>>
bool
equal_values(typename S::container_t lhs, S const& rhs)
{
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}
